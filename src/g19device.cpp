...
void G19Device::initializeDevice() {
  ...
  future = QtConcurrent::run(G19Device::eventThread, this);
...
void G19Device::openDevice() {
  ...
  if (!isInitialized)
  {
      qDebug() << "Not initialised before openDevice call!";
      return;
  }
...
void G19Device::eventThread(G19Device *self) {
  struct timeval tv;

  while (self->enableEventThread) {
    tv.tv_sec = 1;
    tv.tv_usec = 0;
    libusb_handle_events_timeout(self->context, &tv);
  }
}
...
void G19Device::updateLcd(QImage *imgin) {
  ...
  memcpy( dataBuff, hdata, 512 );
  QTransform trans;
  trans.rotate(-90);
  QImage conv = imgin->convertToFormat(QImage::Format_RGB16).transformed(trans).mirrored(false, true);
  memcpy( &dataBuff[512], conv.constBits(), 2*320*240);
